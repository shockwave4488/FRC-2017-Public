        ...
        if (frameCounter >= 30) {
            ...
            if (mFpsText != null) {
                Runnable fpsUpdater = new Runnable() {
                    public void run() {
                        String changes = BuildConfig.ChangeLog;
                        changes = changes.replaceAll(" file changed, ", " f | ");
                        changes = changes.replaceAll(" files changed, ", " f | ");
                        changes = changes.replaceAll(" insertions", "");
                        changes = changes.replaceAll(" insertion", "");
                        changes = changes.replaceAll(" deletions", "");
                        changes = changes.replaceAll(" deletion", "");
                        if (changes == "") {
                            changes = changes + " No Local Changes";
                        }
                        mFpsText.setText("FPS: " + fps + " | SHA " + BuildConfig.GitHash + " | " + changes);
                    }
                };
                // WHEE
